...
const width = d3.select("#visualization_container").node().clientWidth - margin.left - margin.right;
const height = d3.select("#visualization_container").node().clientHeight - margin.top - margin.bottom;
...
Promise.all([
    d3.json("graph.json"),
    d3.json("node_infos.json"),
    d3.json("paths.json"),
    d3.json("image_urls.json")
]).then(([graph, node_infos, paths, image_urls]) => {

    var text_container = d3.select(".sentences");
