...
* next steps 
- Use text search to find all occurrences of "the Michael Jordan
  of". (first results for [[file:examples/mj.org][Michael Jordan]], [[file:examples/m.org][Michelangelo]], [[file:examples/br.org][Babe Ruth]], [[examples/dq.org][Don
  Quixote]])
- Use the results to improve the identification method.
- Add all examples from NYT as test cases.
- Analyse distribution of regex patterns.
- Try to restrict to search for 2nd and 3rd part.
...
|  sum |  246 |  224 |
| mean | 11.7 | 13.2 |
#+TBLFM: @27$2=vsum(@I..@II)::@27$3=vsum(@I..@II)::@28$2=vmean(@I..@II);%2.1f::@28$3=vmean(@I..@II);%2.1f

#+begin_src gnuplot :var data=year-data :file vossantos.svg :results silent
set datafile separator "\t"
set xlabel "Jahr"
set ylabel "Vossantos"
set yrange [0:*]
set ytics 5
...
set out "vossantos.svg"
plot data using ($1+jitter):2 with boxes title 'New York Times',\
     data using ($1-jitter):3 with boxes title 'Die Zeit'

set boxwidth .8 relative

set term svg enhanced size 800,600 dynamic fname "Palatino Linotype, Book Antiqua, Palatino, FreeSerif, serif" fsize 16
set out "vossantos_nyt.svg"
plot data using 1:2 with boxes title ''
set term png enhanced size 800,600 font "Arial,16" lw 3
set out "vossantos_nyt.png"
replot

set term svg enhanced size 800,600 dynamic fname "Palatino Linotype, Book Antiqua, Palatino, FreeSerif, serif" fsize 16
set out "vossantos_zeit.svg"
plot data using 1:3 with boxes title ''
set term png enhanced size 800,600 font "Arial,16" lw  2
set out "vossantos_zeit.png"
replot 
...
#+BEGIN_SRC sh
    grep ^- vossanto-NYT-only-individuals.md | grep -v "<strike>" | sed -e "s/[^\*]*\*[^\*]*\*[^\*]*\*//" -e "s/\*.*//" | sort | uniq -c | sort -nr \
...
* POS patterns
Which are the most frequent POS patterns?

Create a directory which contains only the 500+ NYT articles:
#+BEGIN_SRC sh :results silent
  mkdir v01_nyt_articles
  for i in $(grep ^- vossanto-NYT-only-individuals.md | grep -v "<strike>" | sed -e "s/.*(/nyt_corpus_/" -e "s/)/.xml/" -e "s/\//.har\//"); do
      cp $i v01_nyt_articles
  done
#+END_SRC

Extract the POS/NER patterns:
#+BEGIN_SRC sh
  ./nyt.py --pattern v01_nyt_articles > v01_nyt_patterns.tsv
#+END_SRC

Clean them up and sort them:
#+BEGIN_SRC sh :sep "\t"
   awk -F'\t' '{print $5}' v01_nyt_patterns.tsv |\
  ./analyse_patterns.py | sort | uniq -c | sort -nr | head
#+END_SRC

| frequency | target, | the | source       | of           | modifier |     | example                                                                                  |
|-----------+---------+-----+--------------+--------------+----------+-----+------------------------------------------------------------------------------------------|
|        13 | PERSON, | DT  | ORGANIZATION | IN           | NNS      |     | (PERSON Thomas Eisner) ,/, the/DT (ORGANIZATION St. Francis) of/IN bugs/NNS              |
|        13 | PERSON, | DT  | ORGANIZATION | IN           | GPE      |     | (PERSON Ethel Merman) ,/, the/DT (ORGANIZATION Birgit Nilsson) of/IN (GPE Broadway)      |
|        10 | PERSON, | DT  | ORGANIZATION | IN           | NN       |     | (PERSON Eddie Shore) ,/, the/DT (ORGANIZATION Babe Ruth) of/IN hockey/NN                 |
|         9 | PERSON, | DT  | ORGANIZATION | IN           | DT       | NN  | (PERSON Hal Schell) ,/, the/DT (ORGANIZATION Boswell) of/IN the/DT delta/NN              |
|         7 | PERSON  | VBZ | DT           | ORGANIZATION | IN       | NN  | (PERSON Mr. Solerwitz) is/VBZ the/DT (ORGANIZATION Babe Ruth) of/IN ripoffs/NNS          |
|         7 | PERSON, | DT  | PERSON       | IN           | GPE      |     | (PERSON Oscar) ,/, the/DT (PERSON Larry Bird) of/IN (GPE Brazil) ,/,                     |
|         7 | PERSON, | DT  | ORGANIZATION | IN           | NN       | NNS | (PERSON Edward Rogoff) ,/, the/DT (ORGANIZATION Don Quixote) of/IN cab/NN riders/NNS     |
|         6 | PERSON, | DT  | ORGANIZATION | IN           | JJ       | NN  | (PERSON Carlos Gardel) ,/, the/DT (ORGANIZATION Elvis) of/IN tango/JJ culture/NN         |
|         5 | PERSON, | DT  | ORGANIZATION | IN           | NN       | NN  | (PERSON Johnny Miller) ,/, the/DT (ORGANIZATION Simon Cowell) of/IN golf/NN criticism/NN |
|         5 | GPE,    | DT  | ORGANIZATION | IN           | NNS      |     | (GPE McEnroe) ,/, the/DT (ORGANIZATION Picasso) of/IN players/NNS                        |
