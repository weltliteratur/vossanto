...
#+BEGIN_SRC sh :results raw output
  for term in apostle blues boys capital counterpart czar darling dean diva duke emperor father giant god king land lord marquis master mecca pioneer princess queen satan sultan valley voice; do
      echo "- $term ::"
      grep -i $term README.md | sed "s/^/  /"
  done
#+END_SRC
