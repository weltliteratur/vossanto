...
After second cleansing round -- only individuals are included.
...
#+BEGIN_SRC python :results output table raw
  year = None
  count = None
  with open("vossanto-NYT-only-individuals.md", "r") as f:
      for line in f:
          # find "## Examples"
          if line.startswith("## Examples"):
              year = 0
          ...
          if year is not None and line.startswith("### "):
              ...
              continue
          # find Vossantos
          if year is not None and year > 0 and line.startswith("- ") and not line.startswith("- <strike>"):
              count += 1
              continue
      print(year, count, sep='\t')
#+END_SRC
