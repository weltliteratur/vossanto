...
prediction_file = "wd_sample.tsv"
prediction_file_2 = "ner_sample.tsv"
...
def preprocess_wd(file_path):
    with open(file_path, "r") as file:
        reader = csv.reader(file, delimiter="\t")
        sents = [line[5] for line in reader]
        return sents


def preprocess_ner(file_path):
    with open(file_path, "r") as file:
        reader = csv.reader(file, delimiter="\t")
        sents = [line[3] for line in reader]
        return sents
...
prediction_set = preprocess_wd(prediction_file)
prediction_set_2 = preprocess_ner(prediction_file_2)
print(prediction_set)
print("data size: ", len(vossantos))
print("prediction set size: ", len(prediction_set), len(prediction_set_2))
...
for sent in prediction_set_2:
    ...
    for word in statement_text:
        if word not in vocab_w2v:
            X_inst_p2.append(vocab_w2v["UNK"])
        else:
            X_inst_p2.append(vocab_w2v[word])
    X_predict2.append(X_inst_p2)
...
X_predict = pad_sequences(
    X_predict, maxlen=max_len, value=vocab_w2v["UNK"], padding="pre"
)
X_predict2 = pad_sequences(
    X_predict2, maxlen=max_len, value=vocab_w2v["UNK"], padding="pre"
)
...
tensor = Embedding(
    len(embeddings), emb_dim, weights=[embeddings], input_length=max_len, trainable=True
)(
    main_input
...
# conf_matrix = confusion_matrix(y_train, tests)
...
predictions_2 = model.predict(X_predict2)
# count=0
...
print("# 'true' in predictionsets: ", count_true, count_true_2)
print("# 'false' in predictionsets: ", count_false, count_false_2)
