...
#+BEGIN_SRC sh :results silent
  for year in $(seq 1988 1988); do
    ./check_wikidata.py theof_${year}.tsv wikidata_humans.tsv > theof_${year}_wdfiltered.tsv
  done
#+END_SRC
...
#+BEGIN_SRC sh
    ...
    for year in $(seq 1987 2007); do
      echo $year \
           ...
           $(cat theof_${year}_wdfiltered.tsv | wc -l) \
           $(cat theof_${year}_wdfiltered_blfiltered.tsv | wc -l)
    done
#+END_SRC
...
#+BEGIN_SRC sh
  ...
  ./check_wikidata.py --blacklist blacklist.tsv theof_${YEAR}.tsv wikidata_humans.tsv > theof_${YEAR}_wdfiltered_blfiltered.tsv

  # add all terms from last year to a list
  LAST_YEAR=$((YEAR-1))
  awk -F'\t' '{print $3}' theof_${LAST_YEAR}_wdfiltered_blfiltered.tsv | sort | uniq -c | sort -nr | \
